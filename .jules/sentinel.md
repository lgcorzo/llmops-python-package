...
**Prevention:** In sandbox tools and general file operations, wrap `os` calls (`os.remove`, `os.makedirs`, `open`) in `try...except OSError` blocks and return generic failure messages without referencing `e.strerror` or leaking internal paths/stack traces.
## 2026-03-24 - [Information Leakage via Compiler Exceptions in MCP Tools]
**Vulnerability:** The `execute_code` MCP tool was capturing and returning the raw error message (`e.msg`) from `py_compile.PyCompileError` when compiling Python code in the sandbox. This raw message contains the absolute file path generated by `tempfile.mkdtemp`, effectively leaking internal sandbox directory structures to the client.
**Learning:** Returning raw compiler exception messages (or any raw exception string) directly to clients violates the principle of failing securely. Attackers can use these details to map out the filesystem or understand the underlying environment.
**Prevention:** Always catch specific exceptions from compiler or OS-level operations and return a generic validation or failure message (e.g., "SyntaxError in file.") to external clients to prevent information leakage of absolute internal paths.
## 2026-03-29 - [Information Exposure via HTTP Exceptions in MCP Tools]
**Vulnerability:** The `retrieve_context.py` and `index_code.py` MCP tools caught `httpx.HTTPStatusError` and `httpx.HTTPError` from internal API requests (R2R RAG system) and returned raw attributes like `e.response.text` and `str(e)` to the client. This leaked internal API error messages, tracebacks from downstream services, and detailed server responses to external users.
**Learning:** Returning raw HTTP response text or exception details from internal downstream services directly to clients violates the principle of "Fail securely." Attackers can use these details to map out internal service infrastructure, discover misconfigurations, or gain insights into backend APIs.
**Prevention:** Always catch HTTP exceptions when making requests to internal or downstream services, log the detailed error response server-side (using a secure logger like `loguru`), and return a sanitized, generic error message (e.g., "Internal server error") to the external client.
## 2026-04-06 - [Information Exposure via Exception Details in Job Notifications]
