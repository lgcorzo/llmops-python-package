...
def test_evaluations_job(
    ...
    assert out["targets_lineage"].targets == schemas.TargetsSchema.response, (
    ...
    assert out["dataset"].targets == schemas.TargetsSchema.response, (
        "Dataset targets should be the target column!"
    )
    assert inputs_reader.path in out["dataset"].source.uri, (
        "Dataset source should contain the inputs path!"
    )
    assert targets_reader.path in out["dataset"].source.uri, (
