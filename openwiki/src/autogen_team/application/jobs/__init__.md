---
title: src/autogen_team/application/jobs/__init__.py
source: src/autogen_team/application/jobs/__init__.py
---

# Document: src/autogen_team/application/jobs/__init__.py

## Module Overview

High-level jobs of the project.

### Purpose
Provides functionality for `__init__`.

### Responsibilities
Handles operations and definitions related to `__init__`.

### Main Workflow
Execution flow defined by the functions and classes in the module.

### Dependencies
- `evaluations.EvaluationsJob`
- `explanations.ExplanationsJob`
- `hatchet_inference.HatchetInferenceJob`
- `inference.InferenceJob`
- `promotion.PromotionJob`
- `training.TrainingJob`
- `tuning.TuningJob`

## Public API

### Exported Classes
None

### Exported Functions
None

## UML Diagram

```plantuml
@startuml
note "No classes in module" as N1
@enduml
```

